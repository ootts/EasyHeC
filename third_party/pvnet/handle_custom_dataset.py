...
def sample_fps_points(data_root, trim=0.0, n_points=8):
    ply_path = os.path.join(data_root, 'model.ply')
    ply_points = read_ply_points(ply_path)
    if trim > 0:
        print(f'trimming with {trim}')
    keep = ply_points[:, 2] >= ply_points[:, 2].min() + trim
    ply_points = ply_points[keep]
    fps_points = fps_utils.farthest_point_sampling(ply_points, n_points, True)
