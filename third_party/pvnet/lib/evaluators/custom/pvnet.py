...
class Evaluator:
    ...
    def icp_refine(self, pose_pred, anno, output, K):
        ...
        R_refined, t_refined = icp_utils.icp_refinement(depth, self.icp_render, pose_pred_tmp[:3, :3],
                                                        pose_pred_tmp[:3, 3], K.copy(),
                                                        (depth.shape[1], depth.shape[0]), depth_only=True,
                                                        max_mean_dist_factor=5.0)
        R_refined, _ = icp_utils.icp_refinement(depth, self.icp_render, R_refined, t_refined, K.copy(),
                                                (depth.shape[1], depth.shape[0]), no_depth=True)
    ...
    def model(self):
        if self._model is None:
            model_path = osp.join(cfg.custom.data_dir, "model.ply")
            self._model = pvnet_data_utils.get_ply_model(model_path)
        return self._model

    @property
    def diameter(self):
        if self._diameter is None:
            self._diameter = np.loadtxt(osp.join(cfg.custom.data_dir,'diameter.txt')).item()
