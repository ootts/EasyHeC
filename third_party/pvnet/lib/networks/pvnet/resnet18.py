...
class Resnet18(nn.Module):
    ...
    def decode_keypoint(self, output, user_defined_mask=None):
        vertex = output['vertex'].permute(0, 2, 3, 1)
        b, h, w, vn_2 = vertex.shape
        vertex = vertex.view(b, h, w, vn_2 // 2, 2)
        mask = torch.argmax(output['seg'], 1)  # tensor: 1,h,w : 0,1
        if user_defined_mask is not None:
            mask = mask & torch.from_numpy(user_defined_mask > 0).bool().cuda()[None]
        if cfg.test.max_comp_mask:
            retval, labels, stats, cent = cv2.connectedComponentsWithStats(mask[0].cpu().numpy().astype(np.uint8))

            maxcomp = np.argmax(stats[1:, 4]) + 1
            mask = labels == maxcomp
            mask = torch.from_numpy(mask)[None].bool().cuda()
        if cfg.test.un_pnp:
    ...
    def forward(self, x, user_defined_mask=None):
        ...
        if not self.training:
            with torch.no_grad():
                self.decode_keypoint(ret, user_defined_mask)
